...
async def love_handler(event: BeforeChatEvent) -> None:
    ...
    if not isinstance(nonebot_event, MessageEvent):
        return
    msg_chat_list = event.message
    msg_list = deepcopy(event.get_send_message().copy())
    response_msg = await tools_caller(msg_list[-1], TOOLS)
            ...
            match function_name:
                ...
                case "change_love_points":
                    func_response = await change_love_points(
                        nonebot_event.user_id, function_args.get("delta_love_points",0)
                    )
